...
| | 功能 | 模組類型 | 說明 |
|---|---|---|---|
| ⚡ | **划詞翻譯** | mode | 選中文字 → 即時翻譯彈出 |
| 🖱️ | **懸浮翻譯** | mode | 按住 Alt 懸停文字，無需選中 |
| 📝 | **選詞即譯** | mode | 選中文字後自動翻譯，浮窗展示 |
| 📷 | **截圖翻譯** | 內置 | 擷取螢幕區域 → OCR → 翻譯 |
| 🌐 | **多引擎對比** | translator | 同時展示 Google / LLM 等翻譯結果 |
| 🧹 | **文字淨化** | processor | 自動清理多餘空白、換行、HTML 實體 |
| 🛡️ | **程式碼保護** | processor | 保護程式碼塊不被翻譯引擎破壞 |
| 💾 | **翻譯快取** | service | LRU 快取，相同文字加速 |
| 📚 | **歷史與生詞本** | service | 自動儲存翻譯歷史，收藏生詞 |
| 🌙 | **暗色模式** | style | 跟隨系統自動切換，全面板支援 |

## 🧩 模組系統（核心架構）

QuickTranslate 3.0 引入完整的模組生態。**所有功能都是模組**——劃詞翻譯、懸浮翻譯、選詞即譯、翻譯引擎、文字處理器、背景服務，全部由模組系統統一管理。使用者可以在 🧩 模組系統中自由開關每個模組，無需在多個設定頁面之間切換。
...
| 選詞即譯 | mode | 選中文字自動翻譯 |
| 文字淨化 | processor | 自動清理選中文字的格式 |
| 程式碼保護 | processor | 保護程式碼區塊不被翻譯引擎破壞 |
| 翻譯快取 | service | LRU 快取，200 條加速 |
| 翻譯歷史 | service | 翻譯歷史儲存 |
| 生詞本 | service | 生詞收藏管理 |
...
<p align="center">
  Made with ❤️ by kany2000 · QuickTranslate v3.3.2
</p>
